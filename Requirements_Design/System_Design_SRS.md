...
graph TD
    ...
    OperationsStaff -->|5. Soát vé, vận hành cổng/trò chơi, xử lý sự cố| System
    System -->|6. Trạng thái vận hành, cảnh báo và phân công| OperationsStaff
    ParkManager -->|7. Quản lý hoạt động, báo cáo và hiệu quả kinh doanh| System
    System -->|8. Dashboard, BI, báo cáo và dự báo được cấp quyền| ParkManager
    SalesStaff -->|9. Bán vé, CRM, thành viên, khuyến mãi và POS| System
    SystemAdmin -->|10. Quản trị người dùng, quyền, cấu hình và kiểm toán| System
    System -->|11. Yêu cầu thanh toán| PaymentGateway
    PaymentGateway -->|12. Webhook kết quả thanh toán| System
    System -->|13. Gửi mã OTP, tin nhắn khẩn cấp| SMS_Gate
```
...
graph TD
    subgraph Client_Applications
        AdminPortal[Cổng quản trị ReactJS & TS dùng chung]
    ...
    AdminPortal -->|HTTPS / WSS; JWT + permission| Gateway
    MobileApp -->|HTTPS / WSS| Gateway
    POSApp -->|HTTPS / WSS| Gateway
...
flowchart TD
    ...
    actor PM as PARK_MANAGER
    actor SS as SALES_STAFF
    actor OS as OPERATIONS_STAFF
    ...
    SA --> UC5 & UC6 & UC7 & UC8 & UC9 & UC10 & UC11 & UC12 & UC13
    PM --> UC5 & UC6 & UC7 & UC8 & UC9 & UC10 & UC11 & UC12 & UC13
    SS --> UC2 & UC7
    OS --> UC5 & UC6 & UC8 & UC9 & UC10
```

### 2.3.1. Tác nhân hệ thống và các bên liên quan

| Tác nhân / Bên liên quan | Loại | Mô tả |
|:---|:---|:---|
| Khách hàng / Khách tham quan | Người dùng cuối bên ngoài | Đăng ký thành viên, đặt và sử dụng dịch vụ công viên, nhận vé và thông báo, đồng thời gửi phản hồi. Đây không phải là vai trò nội bộ của Cổng quản trị. |
| `SYSTEM_ADMIN` | Vai trò nội bộ trên Cổng quản trị | Có toàn quyền đối với quản trị hệ thống, người dùng, vai trò, quyền, cấu hình, nhật ký kiểm toán, bảo mật, dashboard, BI, báo cáo, xuất dữ liệu và toàn bộ phân hệ nghiệp vụ. |
| `PARK_MANAGER` | Vai trò nội bộ trên Cổng quản trị | Quản lý vận hành công viên và hiệu quả kinh doanh; được truy cập các chức năng nghiệp vụ, vận hành, BI, báo cáo và xuất dữ liệu nhưng không được truy cập nhóm Quản trị hệ thống. |
| `SALES_STAFF` | Vai trò nội bộ trên Cổng quản trị | Xử lý bán vé, đặt chỗ, đơn hàng, thanh toán, CRM, thành viên, khách hàng thân thiết, khuyến mãi, thương mại, POS, thông báo và hỗ trợ. |
| `OPERATIONS_STAFF` | Vai trò nội bộ trên Cổng quản trị | Xử lý soát vé, cổng, khách tham quan, trò chơi, bãi xe, tủ đồ, thông báo, hỗ trợ và sự cố an toàn. Danh mục trò chơi chỉ được đọc khi áp dụng. |

Bốn tác nhân nội bộ nêu trên là các vai trò duy nhất được phê duyệt cho Cổng quản trị. Trách nhiệm báo cáo điều hành được ánh xạ sang `PARK_MANAGER`; trách nhiệm chăm sóc khách hàng, bán vé và thu ngân được ánh xạ sang `SALES_STAFF`; trách nhiệm vận hành, an ninh, bãi xe, tủ đồ, an toàn và vận hành bảo trì được ánh xạ sang `OPERATIONS_STAFF`; trách nhiệm quản trị chung trước đây được ánh xạ sang `SYSTEM_ADMIN`.

### 2.3.2. Cổng quản trị dùng chung với điều hướng theo vai trò

* **FR-PORTAL-01:** Hệ thống phải tái sử dụng Cổng quản trị dùng chung hiện có và không được tạo website hoặc ứng dụng quản trị riêng cho từng vai trò.
* **FR-PORTAL-02:** `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF` và `OPERATIONS_STAFF` phải sử dụng cùng một bố cục quản trị. Giao diện, bố cục, thanh bên, tiêu đề, breadcrumb và cấu trúc route hiện tại phải được giữ nguyên, trừ khi cần tái cấu trúc nhỏ để tập trung hóa cơ chế phân quyền.
* **FR-PORTAL-03:** Các mục trên thanh bên phải được tạo từ metadata điều hướng tập trung và được lọc động theo quyền hiệu lực của người dùng đã xác thực. Nhóm menu cha phải bị ẩn khi không có mục con nào được cấp quyền.
* **FR-PORTAL-04:** Route, tab, widget dashboard, nút, thao tác trên từng dòng bảng, thao tác hàng loạt và thao tác gửi biểu mẫu không được cấp quyền phải bị ẩn hoặc vô hiệu hóa ở frontend, đồng thời bị backend từ chối độc lập.
* **FR-PORTAL-05:** Phải tái sử dụng các component trang và route hiện có. Không được nhân bản trang hoặc route theo vai trò.
* **FR-PORTAL-06:** Việc ẩn điều hướng hoặc control chỉ là biện pháp hỗ trợ khả dụng và không thay thế việc kiểm tra quyền ở backend.

#### Kiến trúc thông tin thanh điều hướng bên

| Nhóm | Mục menu |
|:---|:---|
| Tổng quan | Dashboard |
| Phân tích và Báo cáo | Dashboard Điều hành; Phân tích và BI; Dashboard Vận hành; Báo cáo; Trung tâm Xuất dữ liệu |
| Vé và Bán hàng | Vé; Soát vé; Đặt chỗ; Đơn hàng; Thanh toán; POS |
| Khách hàng và CRM | Khách hàng; Thành viên; Khách hàng thân thiết; Chiến dịch; Khuyến mãi; Coupon; Voucher; Khách tham quan |
| Vận hành Công viên | Trò chơi; Danh mục Trò chơi; Cổng; Bãi xe; Tủ đồ |
| Thương mại | Khu ẩm thực; Bán lẻ |
| Chăm sóc Khách hàng | Thông báo; Hỗ trợ; Sự cố |
| Quản trị Hệ thống | Người dùng; Vai trò; Quyền; Cấu hình Hệ thống; Nhật ký Kiểm toán |

Chỉ các nhóm và mục được cấp quyền mới được hiển thị. Kiến trúc thông tin này tổ chức lại thanh bên hiện có và không yêu cầu thiết kế lại giao diện trực quan.
...
graph TD
    subgraph User_Interface
        UI[Cổng quản trị ReactJS dùng chung & Looker Studio]
    end
...
sequenceDiagram
    ...
    PM->>Looker: Yêu cầu tải Dashboard Điều hành
    ...
    Looker-->>PM: Hiển thị biểu đồ báo cáo tài chính trực quan
```
...
* **Objective:** Xác thực danh tính người dùng hiện tại cho toàn bộ nhân viên và khách hàng thông qua JWT (JSON Web Token) kết hợp xác thực hai yếu tố (2FA). Xác thực chỉ xác định người dùng; việc quyết định người dùng được phép thực hiện những gì thuộc Module 2 — Phân quyền truy cập.
* **Business Rules:**
...
* **Validation & Errors:** Trả về mã lỗi `ERR-AUTH-001` nếu thông tin đăng nhập sai lệch, `ERR-AUTH-002` nếu tài khoản đang trong trạng thái khóa. HTTP `401 Unauthorized` được dùng khi thiếu, không hợp lệ hoặc hết hạn thông tin xác thực; frontend phải kích hoạt luồng làm mới token hiện có và chỉ yêu cầu đăng nhập lại khi làm mới token thất bại.
...
* **Objective:** Kiểm tra quyền truy cập dựa trên vai trò và các quyền hiệu lực của người dùng đã xác thực tại frontend, API Gateway và từng endpoint nghiệp vụ Spring Boot.
* **Business Rules:**
  ...
  * **BR-AUTHZ-02:** Backend phải áp dụng Spring Security và `@PreAuthorize` (hoặc chính sách tương đương tại ranh giới dịch vụ) cho mọi endpoint được bảo vệ; việc kiểm tra tại Gateway không thay thế kiểm tra ở backend.
  * **BR-AUTHZ-03:** Frontend phải điều khiển menu, route, tab, widget, nút và thao tác bằng permission; không được phân tán các phép kiểm tra vai trò được mã hóa cứng trong component.
  * **BR-AUTHZ-04:** Việc ẩn menu hoặc control không thay thế cơ chế phân quyền ở backend.
* **Use Case Flows:** Người dùng gửi yêu cầu kèm JWT -> Gateway xác thực token -> Backend nạp các quyền hiệu lực từ vai trò của người dùng -> Spring Security/`@PreAuthorize` kiểm tra permission -> Chấp nhận hoặc trả lỗi phù hợp.
* **Validation & Errors:** HTTP `401 Unauthorized` áp dụng khi thiếu, không hợp lệ hoặc hết hạn thông tin xác thực. HTTP `403 Forbidden` và `ERR-AUTHZ-403` áp dụng khi người dùng đã xác thực nhưng thiếu permission. Phản hồi 403 phải hiển thị trang Từ chối truy cập và không đăng xuất hoặc xóa phiên đăng nhập.
* **Access Control:** Áp dụng phòng thủ nhiều lớp tại bộ bảo vệ route ở frontend, API Gateway và cơ chế phân quyền cho method/endpoint ở backend.
* **Database Mapping:** Bảng `user_roles`, `role_permissions`.
* **API Endpoints:**
  * `GET /api/v1/authz/check-permission` (Headers: Authorization token)

#### Yêu cầu bảo vệ route

* **FR-AUTHZ-ROUTE-01:** Hệ thống phải ngăn người dùng không được cấp quyền truy cập route được bảo vệ bằng cách nhập URL trực tiếp.
* **FR-AUTHZ-ROUTE-02:** Hệ thống phải chuyển người dùng đã xác thực nhưng không có quyền tới trang Từ chối truy cập hoặc dashboard đầu tiên được phép mà không hủy phiên xác thực.
* **FR-AUTHZ-ROUTE-03:** Hệ thống phải giữ nguyên các route hiện có và không được tạo route trùng lặp theo vai trò.
* **FR-AUTHZ-ROUTE-04:** Việc kiểm tra xác thực và kiểm tra phân quyền phải được triển khai tách biệt.

#### Phân quyền ở cấp thao tác

* **FR-AUTHZ-ACTION-01:** Các thao tác tạo, sửa, xóa, phê duyệt, hoàn tiền, xuất dữ liệu, cập nhật trạng thái, thao tác hàng loạt, thao tác trên từng dòng bảng, gửi biểu mẫu, truy cập tab hạn chế và widget dashboard nhạy cảm phải yêu cầu permission tương ứng.
* **FR-AUTHZ-ACTION-02:** Người dùng có thể có quyền xem trang nhưng không có quyền cập nhật. Frontend phải ẩn hoặc vô hiệu hóa các control không được cấp quyền, đồng thời API backend vẫn phải từ chối yêu cầu giả mạo hoặc yêu cầu trực tiếp bằng HTTP 403.
* **FR-AUTHZ-ACTION-03:** Các thao tác xuất dữ liệu nhạy cảm, hoàn tiền, phê duyệt và thay đổi cấu hình hệ thống phải được ghi nhật ký kiểm toán dù được cho phép hay bị từ chối.

### MODULE 3: ROLE & PERMISSION (VAI TRÒ & PHÂN QUYỀN)
* **Objective:** Cho phép `SYSTEM_ADMIN` quản lý tập trung bốn vai trò được phê duyệt, gán tập hợp permission và áp dụng động trong cùng một Cổng quản trị.
* **Business Rules:**
  * **BR-ROLE-01:** Không cho phép xóa các vai trò hệ thống cốt lõi (`SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF`, `OPERATIONS_STAFF`).
  * **BR-ROLE-02:** Vai trò là tập hợp các permission. Mã permission phải theo quy ước `RESOURCE_ACTION`, viết hoa, dùng dấu gạch dưới và là duy nhất.
  * **BR-ROLE-03:** Danh mục permission được đăng ký tập trung. Module mới phải đăng ký permission tại danh mục này; frontend không được mã hóa cứng và phân tán các phép kiểm tra vai trò.
  * **BR-ROLE-04:** Smart Park MVP cấp đúng một vai trò chính cho mỗi nhân viên. Cấu trúc `user_roles` được giữ để tương thích, nhưng phải có ràng buộc duy nhất theo `user_id` trong MVP; việc mở rộng đa vai trò về sau phải có yêu cầu thay đổi.
* **Use Case Flows:** `SYSTEM_ADMIN` chọn một vai trò -> Tích chọn danh sách permission trên ma trận phân quyền -> Lưu -> Hệ thống áp dụng cho phiên mới hoặc phiên được làm mới và ghi nhật ký kiểm toán.
* **Validation & Errors:** Gửi yêu cầu xóa vai trò hệ thống trả về lỗi `ERR-ROLE-001`.
* **Access Control:** Yêu cầu xác thực; vai trò được phép là `SYSTEM_ADMIN`; permission bắt buộc theo từng thao tác gồm `ROLE_VIEW`, `ROLE_CREATE`, `ROLE_UPDATE`, `ROLE_DELETE`, `PERMISSION_VIEW`, `PERMISSION_ASSIGN`.
...
#### Danh mục permission tập trung

Danh mục tối thiểu gồm: `DASHBOARD_VIEW`, `ANALYTICS_VIEW`, `REPORT_VIEW`, `REPORT_EXPORT`, `USER_VIEW`, `USER_CREATE`, `USER_UPDATE`, `USER_DELETE`, `ROLE_VIEW`, `ROLE_CREATE`, `ROLE_UPDATE`, `ROLE_DELETE`, `PERMISSION_VIEW`, `PERMISSION_ASSIGN`, `TICKET_VIEW`, `TICKET_CREATE`, `TICKET_UPDATE`, `TICKET_VALIDATE`, `BOOKING_VIEW`, `BOOKING_CREATE`, `ORDER_VIEW`, `ORDER_UPDATE`, `PAYMENT_VIEW`, `PAYMENT_CREATE`, `PAYMENT_REFUND`, `CUSTOMER_VIEW`, `CUSTOMER_UPDATE`, `MEMBERSHIP_VIEW`, `MEMBERSHIP_UPDATE`, `RIDE_VIEW`, `RIDE_UPDATE`, `GATE_VIEW`, `GATE_CONTROL`, `PARKING_VIEW`, `PARKING_MANAGE`, `LOCKER_VIEW`, `LOCKER_MANAGE`, `POS_VIEW`, `POS_CREATE`, `SUPPORT_VIEW`, `SUPPORT_UPDATE`, `INCIDENT_VIEW`, `INCIDENT_UPDATE`, `SETTINGS_VIEW`, `SETTINGS_UPDATE`, `AUDIT_VIEW`.
...
* **Use Case Flows:** `SYSTEM_ADMIN` truy vấn tài khoản -> Thay đổi thông tin cá nhân, vai trò chính hoặc trạng thái hoạt động -> Lưu thay đổi và ghi nhật ký kiểm toán.
* **Validation & Errors:** Địa chỉ email khi đăng ký mới không được trùng lặp với bất kỳ tài khoản nào đã tồn tại trong DB.
* **Access Control:** Yêu cầu xác thực; vai trò được phép là `SYSTEM_ADMIN`; thao tác `GET` yêu cầu `USER_VIEW`, tạo mới yêu cầu `USER_CREATE`, cập nhật trạng thái hoặc gán vai trò yêu cầu `USER_UPDATE`, và xóa logic yêu cầu `USER_DELETE`.
* **UI Description:** Giao diện quản trị dạng bảng có tính năng tìm kiếm, phân trang và lọc tài khoản theo trạng thái.
* **Database Mapping:** Bảng `users`, `user_roles`.
* **API Endpoints:**
...
* **Access Control:** Yêu cầu xác thực; các vai trò được phép gồm `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF`; đọc yêu cầu `CUSTOMER_VIEW`, cập nhật yêu cầu `CUSTOMER_UPDATE`; trường dữ liệu PII yêu cầu thêm permission xem dữ liệu nhạy cảm.
...
* **Access Control:** Yêu cầu xác thực; các vai trò được phép gồm `SYSTEM_ADMIN`, `PARK_MANAGER`; đọc yêu cầu `EMPLOYEE_VIEW`, tạo hoặc cập nhật yêu cầu `EMPLOYEE_UPDATE`; trường lương yêu cầu thêm `EMPLOYEE_SENSITIVE_VIEW`.
...
* **Use Case Flows:** `SYSTEM_ADMIN` tạo phòng ban mới -> Chọn nhân sự làm quản lý phòng ban -> Lưu thông tin cấu trúc tổ chức.
* **Validation & Errors:** Không được phép xóa phòng ban nếu phòng ban đó đang chứa nhân sự hoạt động.
* **Access Control:** Yêu cầu xác thực; vai trò được phép là `SYSTEM_ADMIN`; yêu cầu permission phòng ban tương ứng cho thao tác xem, tạo, cập nhật hoặc xóa.
...
* **Access Control:** Yêu cầu xác thực; vai trò được phép là `SYSTEM_ADMIN`; yêu cầu permission công viên tương ứng cho thao tác xem, tạo, cập nhật hoặc xóa.
...
* **Access Control:** Yêu cầu xác thực; các vai trò được phép gồm `SYSTEM_ADMIN`, `PARK_MANAGER`; đọc yêu cầu `ZONE_VIEW`, thay đổi dữ liệu yêu cầu `ZONE_UPDATE`.
...
* **Access Control:** Yêu cầu xác thực; các vai trò được phép gồm `SYSTEM_ADMIN`, `PARK_MANAGER`, `OPERATIONS_STAFF`; đọc yêu cầu `RIDE_VIEW`, cập nhật trạng thái yêu cầu `RIDE_UPDATE`.
...
* **Access Control:** Yêu cầu xác thực; vai trò cũ `Maintenance Tech` được thay thế bằng `OPERATIONS_STAFF`; các vai trò được phép gồm `SYSTEM_ADMIN`, `PARK_MANAGER`, `OPERATIONS_STAFF`; đọc yêu cầu `RIDE_VIEW` và thao tác bảo trì yêu cầu `RIDE_UPDATE`.
...
* **Business Rules:**
  * **BR-PRICE-01:** Biên độ điều chỉnh giá vé động của AI không được vượt quá khoảng giới hạn tối thiểu (`min_price`) và tối đa (`max_price`) do `SYSTEM_ADMIN` thiết lập tĩnh cho từng loại vé trong Spring Boot.
...
* **Business Rules:**
  ...
  * **BR-BI-02:** Áp dụng cơ chế Dead Letter Queue (DLQ) cho Analytics Outbox; các sự kiện retry quá 3 lần sẽ được chuyển vào bảng Dead Letter để `SYSTEM_ADMIN` quản lý và đưa trở lại hàng đợi khi cần.
...
* **Objective:** Giao diện Google Looker Studio truy vấn dữ liệu từ BigQuery và trực quan hóa chỉ số tài chính, vận hành thời gian thực cho `SYSTEM_ADMIN` và `PARK_MANAGER` có `ANALYTICS_VIEW`/`REPORT_VIEW`.
...
### 3.36. Ma trận quyền truy cập menu theo vai trò

`Toàn quyền` nghĩa là được thực hiện mọi thao tác xem và thay đổi đã cấp cho phân hệ; `Chỉ đọc` nghĩa là chỉ được xem và không được thay đổi; `Hạn chế` nghĩa là chỉ được thực hiện các thao tác vận hành hoặc bán hàng đã cấp rõ ràng bằng permission; `Không truy cập` nghĩa là không được truy cập cả menu lẫn route.

| Phân hệ | SYSTEM_ADMIN | PARK_MANAGER | SALES_STAFF | OPERATIONS_STAFF |
|:---|:---:|:---:|:---:|:---:|
| Dashboard | Toàn quyền | Toàn quyền | Hạn chế | Hạn chế |
| Phân tích và BI | Toàn quyền | Toàn quyền | Không truy cập | Không truy cập |
| Báo cáo | Toàn quyền | Toàn quyền | Hạn chế | Không truy cập |
| Xuất dữ liệu | Toàn quyền | Toàn quyền | Hạn chế | Không truy cập |
| Người dùng | Toàn quyền | Không truy cập | Không truy cập | Không truy cập |
| Vai trò và Quyền | Toàn quyền | Không truy cập | Không truy cập | Không truy cập |
| Cấu hình | Toàn quyền | Không truy cập | Không truy cập | Không truy cập |
| Vé | Toàn quyền | Toàn quyền | Toàn quyền | Hạn chế |
| Soát vé | Toàn quyền | Toàn quyền | Hạn chế | Toàn quyền |
| Đặt chỗ | Toàn quyền | Toàn quyền | Toàn quyền | Không truy cập |
| Đơn hàng | Toàn quyền | Toàn quyền | Toàn quyền | Không truy cập |
| Thanh toán | Toàn quyền | Toàn quyền | Hạn chế | Không truy cập |
| Khách hàng | Toàn quyền | Toàn quyền | Toàn quyền | Không truy cập |
| Thành viên | Toàn quyền | Toàn quyền | Toàn quyền | Không truy cập |
| Khách hàng thân thiết | Toàn quyền | Toàn quyền | Toàn quyền | Không truy cập |
| Khuyến mãi | Toàn quyền | Toàn quyền | Toàn quyền | Không truy cập |
| Trò chơi | Toàn quyền | Toàn quyền | Không truy cập | Hạn chế |
| Cổng | Toàn quyền | Toàn quyền | Không truy cập | Toàn quyền |
| Bãi xe | Toàn quyền | Toàn quyền | Không truy cập | Toàn quyền |
| Tủ đồ | Toàn quyền | Toàn quyền | Không truy cập | Toàn quyền |
| Khu ẩm thực | Toàn quyền | Toàn quyền | Toàn quyền | Không truy cập |
| Bán lẻ | Toàn quyền | Toàn quyền | Toàn quyền | Không truy cập |
| POS | Toàn quyền | Toàn quyền | Toàn quyền | Không truy cập |
| Thông báo | Toàn quyền | Toàn quyền | Hạn chế | Hạn chế |
| Hỗ trợ | Toàn quyền | Toàn quyền | Toàn quyền | Toàn quyền |
| Sự cố | Toàn quyền | Toàn quyền | Không truy cập | Toàn quyền |
| Nhật ký kiểm toán | Toàn quyền | Không truy cập | Không truy cập | Không truy cập |

Chiến dịch, Coupon, Voucher và Khách tham quan tuân theo quyền của miền cha: Chiến dịch/Coupon/Voucher là Toàn quyền đối với `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF` và Không truy cập đối với `OPERATIONS_STAFF`; Khách tham quan là Toàn quyền đối với `SYSTEM_ADMIN`, `PARK_MANAGER`, Hạn chế đối với `SALES_STAFF` và Toàn quyền đối với `OPERATIONS_STAFF`. Danh mục Trò chơi là Toàn quyền đối với `SYSTEM_ADMIN`, `PARK_MANAGER`, Không truy cập đối với `SALES_STAFF` và Chỉ đọc đối với `OPERATIONS_STAFF`. `SALES_STAFF` không được truy cập các báo cáo tài chính nhạy cảm.

### 3.37. Cá nhân hóa dashboard theo vai trò

Hệ thống phải cung cấp một route và một component Dashboard dùng chung. Không được tạo bốn trang dashboard riêng theo vai trò. Khả năng hiển thị widget và phạm vi dữ liệu phải được xác định từ metadata permission:

| Vai trò | Nhóm widget được phép |
|:---|:---|
| `SYSTEM_ADMIN` | KPI hệ thống; Doanh thu; Vận hành; Người dùng; Bảo mật; Phân tích |
| `PARK_MANAGER` | Doanh thu; Vận hành; Khách tham quan; Trò chơi; Bãi xe; Sự cố |
| `SALES_STAFF` | Bán vé; Đơn hàng; Khách hàng; Thành viên; POS |
| `OPERATIONS_STAFF` | Khách hiện tại; Cổng; Trạng thái trò chơi; Bãi xe; Tủ đồ; Sự cố |

* **FR-DASH-01:** Widget nhạy cảm phải yêu cầu permission ở cấp widget ngay cả khi người dùng được truy cập Dashboard.
* **FR-DASH-02:** Không được yêu cầu dữ liệu từ backend hoặc hiển thị placeholder trống cho widget không được cấp quyền.
* **FR-DASH-03:** Việc cá nhân hóa Dashboard phải tái sử dụng bố cục và component dashboard hiện tại.
...
43. **`audit_logs`** (id, user_id, action, resource, target_table, record_id, old_values, new_values, result, ip_address, correlation_id, created_at)
...
#### 6.1.1. Ràng buộc dữ liệu phân quyền

* `roles.code` phải là duy nhất và trong phạm vi MVP chỉ được nhận một trong các giá trị `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF`, `OPERATIONS_STAFF`.
* `permissions.code` phải là duy nhất và tuân theo quy ước `RESOURCE_ACTION`.
* `user_roles` phải có quan hệ `(user_id, role_id)` duy nhất; theo quy tắc một vai trò chính của MVP, `user_id` cũng phải là duy nhất.
* `role_permissions` phải có quan hệ `(role_id, permission_id)` duy nhất.
* Phải tái sử dụng các bảng `users`, `roles`, `permissions`, `user_roles`, `role_permissions` hiện có; không được tạo bảng phân quyền dư thừa.
...
### 7.1.8. Danh mục kiểm soát truy cập API tập trung

Tất cả mục được đánh dấu Yêu cầu xác thực phải được thực thi bằng Spring Security và `@PreAuthorize`. `SYSTEM_ADMIN` mặc nhiên có toàn bộ permission được liệt kê. Các endpoint giữ nguyên đường dẫn hiện có.

| Endpoint hoặc nhóm endpoint | Vai trò được phép | Permission bắt buộc | Mức truy cập | Phạm vi công khai |
|:---|:---|:---|:---|:---|
| `POST /api/v1/auth/login`, `POST /api/v1/auth/mfa/verify` | Mọi người dùng hợp lệ | Không yêu cầu trước khi xác thực | Ghi | Công khai |
| `GET /api/v1/authz/check-permission` | Cả bốn vai trò | Phiên đã xác thực | Đọc | Yêu cầu xác thực |
| `/api/v1/admin/users/**` | `SYSTEM_ADMIN` | `USER_VIEW` / `USER_CREATE` / `USER_UPDATE` / `USER_DELETE` | Đọc/Ghi theo phương thức | Yêu cầu xác thực |
| `/api/v1/admin/roles/**` | `SYSTEM_ADMIN` | `ROLE_CREATE`, `ROLE_UPDATE`, `PERMISSION_ASSIGN` | Ghi | Yêu cầu xác thực |
| `GET /api/v1/admin/audit-logs` | `SYSTEM_ADMIN` | `AUDIT_VIEW` | Đọc | Yêu cầu xác thực |
| `/api/v1/admin/dlq/**` | `SYSTEM_ADMIN` | `ANALYTICS_VIEW` / `ANALYTICS_MANAGE` | Đọc/Ghi | Yêu cầu xác thực |
| `/api/v1/customers/**` | `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF` | `CUSTOMER_VIEW`, `CUSTOMER_CREATE`, `CUSTOMER_UPDATE` | Đọc/Ghi theo phương thức | Yêu cầu xác thực; đăng ký tự phục vụ của khách hàng có thể Công khai |
| `/api/v1/employees/**`, `/api/v1/departments/**` | `SYSTEM_ADMIN`; `PARK_MANAGER` khi được cấp | `EMPLOYEE_VIEW/UPDATE`, `DEPARTMENT_VIEW/UPDATE` | Đọc/Ghi theo phương thức | Yêu cầu xác thực |
| `/api/v1/parks/**` | `SYSTEM_ADMIN`; `PARK_MANAGER` đối với phân khu | `PARK_VIEW/UPDATE`, `ZONE_VIEW/UPDATE` | Đọc/Ghi theo phương thức | Yêu cầu xác thực |
| Nhóm endpoint đọc trò chơi và trạng thái sức chứa | `SYSTEM_ADMIN`, `PARK_MANAGER`, `OPERATIONS_STAFF` | `RIDE_VIEW` | Đọc | Yêu cầu xác thực |
| Trạng thái, lịch, kiểm tra và bảo trì trò chơi | `SYSTEM_ADMIN`, `PARK_MANAGER`, `OPERATIONS_STAFF` | `RIDE_UPDATE` hoặc `INCIDENT_UPDATE` tùy trường hợp | Ghi | Yêu cầu xác thực |
| `POST /api/v1/ride-categories` | `SYSTEM_ADMIN`, `PARK_MANAGER` | `RIDE_UPDATE` | Ghi | Yêu cầu xác thực |
| Loại vé, vé và định giá động | `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF`; `OPERATIONS_STAFF` cũng được soát vé | `TICKET_VIEW/CREATE/UPDATE/VALIDATE` | Đọc/Ghi theo thao tác | Yêu cầu xác thực; đọc danh mục cho khách hàng có thể Công khai |
| `/api/v1/bookings/**` | `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF`; khách hàng tự phục vụ | `BOOKING_VIEW/CREATE`, `PAYMENT_CREATE` | Đọc/Ghi | Yêu cầu xác thực hoặc Công khai đối với luồng tự phục vụ đã phê duyệt |
| Nhóm endpoint đọc đơn hàng | `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF` | `ORDER_VIEW` | Đọc | Yêu cầu xác thực |
| Webhook thanh toán | Tài khoản dịch vụ tin cậy của cổng thanh toán | Chính sách webhook có chữ ký, không phải vai trò quản trị | Ghi | Callback công khai có kiểm tra chữ ký |
| Thành viên, khuyến mãi và coupon | `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF` | Permission `MEMBERSHIP_*` / `PROMOTION_*` tương ứng | Đọc/Ghi theo phương thức | Yêu cầu xác thực |
| Endpoint Khu ẩm thực, Bán lẻ và POS | `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF` | Permission `FOOD_*`, `RETAIL_*`, `POS_*` tương ứng | Đọc/Ghi theo phương thức | Yêu cầu xác thực |
| Nhóm endpoint xe vào/ra bãi | `SYSTEM_ADMIN`, `PARK_MANAGER`, `OPERATIONS_STAFF`; tài khoản dịch vụ IoT tin cậy | `PARKING_MANAGE` hoặc thông tin xác thực thiết bị | Ghi | Yêu cầu xác thực |
| Tủ đồ | `SYSTEM_ADMIN`, `PARK_MANAGER`, `OPERATIONS_STAFF`; khách hàng tự phục vụ khi được phê duyệt | `LOCKER_VIEW/MANAGE` | Đọc/Ghi | Yêu cầu xác thực |
| Gửi phản hồi | Khách hàng/Khách tham quan; `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF` xử lý | Chính sách gửi phản hồi / `SUPPORT_UPDATE` | Ghi | Gửi công khai hoặc xử lý có xác thực |
| Thông báo | Cả bốn vai trò khi đúng phạm vi | `NOTIFICATION_SEND` | Ghi | Yêu cầu xác thực |
| Sự cố | `SYSTEM_ADMIN`, `PARK_MANAGER`, `OPERATIONS_STAFF` | `INCIDENT_VIEW/UPDATE` | Đọc/Ghi | Yêu cầu xác thực |
| Dashboard BI, KPI, dự báo và trạng thái pipeline | `SYSTEM_ADMIN`, `PARK_MANAGER` | `ANALYTICS_VIEW`, `REPORT_VIEW` | Đọc | Yêu cầu xác thực |
| Endpoint suy luận FastAPI | Chỉ danh tính dịch vụ Spring Boot | Phạm vi dịch vụ-đến-dịch vụ tương ứng với thao tác suy luận | Tính toán đọc | Nội bộ; không cung cấp trực tiếp cho Cổng quản trị |
...
* **NFR-PERF-03:** Dữ liệu vai trò và permission phải được nạp một lần từ phiên đã xác thực hoặc hồ sơ người dùng hiện tại; việc lọc thanh bên phải được thực hiện trong bộ nhớ; frontend không được gọi API kiểm tra permission ở mỗi lần render.
...
* **NFR-SEC-02:** Áp dụng nguyên tắc đặc quyền tối thiểu và phòng thủ nhiều lớp bằng RBAC, JWT, bộ bảo vệ frontend, API Gateway, Spring Security và `@PreAuthorize` ở backend; quyết định phân quyền của backend là quyết định có hiệu lực cuối cùng.
* **NFR-SEC-03:** HTTP 401 và 403 phải được xử lý tách biệt. Phản hồi 401 khởi động luồng làm mới token hiện có; phản hồi 403 hiển thị Từ chối truy cập và không được đăng xuất người dùng.
* **NFR-SEC-04:** Phải tạo bản ghi kiểm toán bất biến cho việc gán vai trò, gán permission, thay đổi vai trò, thay đổi permission, cố gắng truy cập trái phép, xuất dữ liệu nhạy cảm, hoàn tiền và thay đổi cấu hình hệ thống.
* **NFR-SEC-05:** Mỗi bản ghi kiểm toán nhạy cảm về permission phải bao gồm người dùng, hành động, tài nguyên, thời điểm, kết quả, địa chỉ IP khi có và mã tương quan khi có.
...
* **NFR-MAINT-01:** Vai trò, permission, metadata điều hướng và chính sách route phải được định nghĩa tập trung; nghiêm cấm các phép kiểm tra vai trò được mã hóa cứng và phân tán.
* **NFR-MAINT-02:** Hệ thống không được nhân bản trang hoặc route theo vai trò. Module mới phải đăng ký permission trong danh mục tập trung và tái sử dụng tiện ích phân quyền dùng chung.

## 8.8. Usability (Khả năng sử dụng)
* **NFR-USE-01:** Người dùng chỉ được nhìn thấy các chức năng liên quan; nhóm cha trên thanh bên phải bị ẩn khi không có mục con nào được cấp quyền.
* **NFR-USE-02:** Phong cách trực quan hiện có phải được duy trì nhất quán; điều hướng được cấp quyền phải dễ hiểu và gọn.
...
## 9.3. Kiểm thử chấp nhận và hồi quy RBAC

| Mã kiểm thử | Tiêu chí chấp nhận |
|:---|:---|
| AT-RBAC-01 | `SYSTEM_ADMIN` nhìn thấy toàn bộ nhóm menu, mục menu, widget và thao tác được phép. |
| AT-RBAC-02 | `PARK_MANAGER` không thể nhìn thấy hoặc truy cập menu, route hay API thuộc nhóm Quản trị Hệ thống. |
| AT-RBAC-03 | `SALES_STAFF` không thể nhìn thấy hoặc truy cập các trang quản lý Trò chơi, Cổng, Bãi xe, Tủ đồ hoặc Sự cố. |
| AT-RBAC-04 | `OPERATIONS_STAFF` không thể nhìn thấy hoặc truy cập Thanh toán, Đơn hàng, quản trị Thành viên, quản trị Khách hàng thân thiết hoặc quản trị Khuyến mãi/CRM. |
| AT-RBAC-05 | Nhóm menu cha bị ẩn khi mọi mục con đều không được cấp quyền. |
| AT-RBAC-06 | Việc nhập trực tiếp URL của route không được cấp quyền phải dẫn đến HTTP 403 và/hoặc trang Từ chối truy cập; dữ liệu được bảo vệ không được tiết lộ. |
| AT-RBAC-07 | Nút, tab, thao tác trên từng dòng, thao tác hàng loạt và widget nhạy cảm không được cấp quyền phải bị ẩn hoặc vô hiệu hóa; lệnh gọi API trực tiếp phải bị từ chối. |
| AT-RBAC-08 | Mọi menu, route và thao tác đã được cấp quyền vẫn phải sử dụng được đối với vai trò tương ứng. |
| AT-RBAC-09 | Phản hồi 403 phải giữ nguyên phiên xác thực và không đăng xuất người dùng. |
| AT-RBAC-10 | Phản hồi 401 phải kích hoạt luồng làm mới token hiện có; chỉ yêu cầu đăng nhập khi làm mới token thất bại. |
| AT-RBAC-11 | Điều hướng, biểu mẫu, bộ lọc, phân trang và hộp thoại hiện có phải tiếp tục hoạt động sau khi áp dụng lọc permission. |
| AT-RBAC-12 | Kiểm thử tích hợp backend phải xác minh `@PreAuthorize` cho cả tổ hợp vai trò-permission được phép và bị từ chối; kiểm thử frontend phải xác minh việc lọc menu, route, widget và thao tác. |

---

# SECTION 10: HỒ SƠ THAY ĐỔI RBAC

## 10.1. Phân tích tác động

| Phần SRS | Vấn đề hiện tại | Thay đổi bắt buộc | Mức tác động |
|:---|:---|:---|:---:|
| 2.1–2.3 Tác nhân và sơ đồ | Các tác nhân Admin, Executive, Operator và Tech chung chung bị chồng chéo, không thể hiện các vai trò Cổng quản trị đã phê duyệt. | Thay tác nhân nội bộ bằng bốn vai trò đã phê duyệt, đồng thời giữ Khách hàng/Khách tham quan là tác nhân bên ngoài. | Cao |
| 2.3.2 Cổng quản trị | Chưa có ràng buộc rõ ràng về Cổng quản trị dùng chung hoặc điều hướng được lọc theo permission. | Bổ sung yêu cầu về bố cục dùng chung, điều hướng tập trung, route được bảo vệ và tái sử dụng component. | Cao |
| Module 1–4 | Xác thực và phân quyền chưa được tách rõ; cơ chế thực thi ở backend và xử lý 401/403 chưa đầy đủ. | Tách xác thực khỏi phân quyền; bổ sung Spring Security, `@PreAuthorize`, yêu cầu route và thao tác. | Cao |
| Module 5–35 | Một số quy tắc truy cập dùng tên vai trò cũ hoặc thiếu ngữ nghĩa permission rõ ràng. | Ánh xạ trách nhiệm cũ sang vai trò đã phê duyệt và permission tài nguyên; giữ nguyên quy tắc nghiệp vụ và endpoint. | Cao |
| 3.36–3.37 Điều hướng và Dashboard | Chưa có ma trận menu hoàn chỉnh hoặc mô hình cá nhân hóa widget trên dashboard dùng chung. | Bổ sung ma trận menu và một dashboard có widget được lọc theo permission. | Cao |
| 6.1 Lược đồ phân quyền | Các bảng RBAC đã có nhưng chưa nêu rõ tính duy nhất, chính sách một vai trò chính và trường dữ liệu kiểm toán. | Tái sử dụng bảng; bổ sung ràng buộc, số lượng vai trò trong MVP và metadata kiểm toán. | Trung bình |
| 7.1 Thiết kế API | Endpoint chưa có danh mục tập trung về vai trò được phép, permission, mức đọc/ghi và phạm vi công khai. | Bổ sung danh mục kiểm soát truy cập mà không thay đổi đường dẫn endpoint. | Cao |
| 8 Yêu cầu phi chức năng | Chưa đầy đủ về đặc quyền tối thiểu, khả năng bảo trì, khả năng sử dụng và hiệu năng phân quyền. | Bổ sung phòng thủ nhiều lớp, permission lưu theo phiên, chính sách tập trung và điều hướng gọn. | Trung bình |
| 9 Kiểm thử | Chưa có bộ tiêu chí chấp nhận/hồi quy RBAC theo vai trò. | Bổ sung tiêu chí kiểm thử cho luồng hợp lệ, không hợp lệ, 401/403 và hồi quy. | Trung bình |

## 10.2. Ánh xạ vai trò

| Tên vai trò hiện có | Vai trò mới được phê duyệt | Lý do |
|:---|:---|:---|
| SUPER_ADMIN | `SYSTEM_ADMIN` | Chịu trách nhiệm toàn bộ hệ thống, bảo mật và quản trị. |
| ADMIN / Admin chung | `SYSTEM_ADMIN` | Các trường hợp hiện có liên quan đến cấu hình hệ thống, người dùng, vai trò, giới hạn giá hoặc quản trị pipeline dữ liệu. |
| BUSINESS_ANALYST | `PARK_MANAGER` | Giả định áp dụng trong ngữ cảnh BI/báo cáo; chỉ dùng `SYSTEM_ADMIN` khi ngữ cảnh là cấu hình hệ thống. |
| Executive / Ban Giám đốc | `PARK_MANAGER` | Sử dụng báo cáo điều hành, vận hành và hiệu quả kinh doanh. |
| CRM_MANAGER / CUSTOMER_SERVICE | `SALES_STAFF` | Chịu trách nhiệm về khách hàng, thành viên và hỗ trợ. |
| TICKET_STAFF / CASHIER | `SALES_STAFF` | Chịu trách nhiệm về vé, thanh toán và POS. |
| OPERATOR / SECURITY_STAFF / PARKING_OPERATOR / LOCKER_OPERATOR / SAFETY_STAFF | `OPERATIONS_STAFF` | Chịu trách nhiệm vận hành công viên, kiểm soát ra vào và sự cố. |
| Maintenance Tech / nhân viên kỹ thuật vận hành | `OPERATIONS_STAFF` | Giả định nhiệm vụ vận hành bảo trì thuộc vai trò vận hành đã phê duyệt; thao tác nhạy cảm vẫn yêu cầu permission rõ ràng. |
| PARK_MANAGER | `PARK_MANAGER` | Đã được phê duyệt và giữ nguyên. |

## 10.3. Nội dung SRS được sửa chính xác

### Phần 2.1–2.3 — Tác nhân, Cổng quản trị dùng chung và Điều hướng

**Nội dung hiện tại được thay thế:** Các tác nhân nội bộ chung chung và một web client ReactJS không phân biệt vai trò.

**Nội dung sửa đổi:** Các tác nhân nội bộ của Cổng quản trị chỉ gồm `SYSTEM_ADMIN`, `PARK_MANAGER`, `SALES_STAFF`, `OPERATIONS_STAFF`; Khách hàng/Khách tham quan vẫn là tác nhân bên ngoài. Cổng quản trị ReactJS dùng chung, bố cục, giao diện, thanh bên, tiêu đề, breadcrumb, route và component trang hiện có được tái sử dụng. Menu và nhóm cha được lọc từ metadata permission tập trung; route/thao tác trái phép bị chặn; không nhân bản website, trang hoặc route theo vai trò.

### Module 1–2 — Xác thực và Phân quyền

**Nội dung hiện tại được thay thế:** Trích xuất vai trò từ JWT và chỉ kiểm tra quyền tại Gateway với một mô tả lỗi 403 duy nhất.

**Nội dung sửa đổi:** Xác thực xác định danh tính người dùng; phân quyền đánh giá permission của vai trò một cách độc lập. HTTP 401 nghĩa là thiếu, không hợp lệ hoặc hết hạn thông tin xác thực và kích hoạt luồng làm mới token hiện có. HTTP 403 nghĩa là người dùng đã xác thực nhưng thiếu permission, hiển thị Từ chối truy cập và không kết thúc phiên. Bộ bảo vệ frontend, kiểm tra Gateway và Spring Security/`@PreAuthorize` ở backend cùng thực thi phòng thủ nhiều lớp; việc ẩn control không bao giờ thay thế kiểm tra quyền ở backend.

### Module 3–4 — Quản lý Vai trò, Permission và Người dùng

**Nội dung hiện tại được thay thế:** Cách diễn đạt System Admin chung chung và hai vai trò lõi được bảo vệ nhưng chưa có quy ước đặt tên hoặc số lượng vai trò của nhân viên.

**Nội dung sửa đổi:** `SYSTEM_ADMIN` quản trị đúng bốn vai trò lõi. Vai trò là tập hợp permission; mã permission sử dụng tên `RESOURCE_ACTION` duy nhất và được đăng ký tập trung. MVP gán một vai trò chính cho mỗi nhân viên trong khi vẫn giữ các bảng RBAC hiện có. Thao tác với vai trò, permission, người dùng và gán vai trò phải yêu cầu permission tương ứng và tạo bản ghi kiểm toán bất biến.

### Phần 3.36–3.37 — Menu và Dashboard

**Nội dung hiện tại được thay thế:** Chưa có ma trận vai trò-menu hợp nhất và định nghĩa dashboard tập trung vào vai trò điều hành.

**Nội dung sửa đổi:** Ma trận vai trò-menu sử dụng các giá trị Toàn quyền, Chỉ đọc, Hạn chế và Không truy cập. Một route và một component Dashboard dùng chung chỉ hiển thị các nhóm widget được cấp quyền cho vai trò hiện tại; widget nhạy cảm yêu cầu permission độc lập; widget trái phép không được tải dữ liệu hoặc render.

### Phần 6.1 — Mô hình dữ liệu phân quyền

**Nội dung hiện tại được thay thế:** Các bảng `users`, `roles`, `permissions`, `user_roles`, `role_permissions` hiện có nhưng chưa có ràng buộc RBAC rõ ràng.

**Nội dung sửa đổi:** Tái sử dụng các bảng hiện có. Mã vai trò, mã permission, quan hệ người dùng-vai trò và quan hệ vai trò-permission phải là duy nhất; `user_id` trong `user_roles` là duy nhất theo chính sách một vai trò chính của MVP. Bản ghi kiểm toán bao gồm người dùng, hành động, tài nguyên, thời điểm/kết quả, địa chỉ IP và mã tương quan khi có.

### Phần 7.1.8 — Kiểm soát truy cập API

**Nội dung hiện tại được thay thế:** Danh sách endpoint theo miền chưa có thông tin nhất quán về vai trò, permission, mức đọc/ghi và phạm vi công khai.

**Nội dung sửa đổi:** Danh mục tập trung xác định vai trò được phép, permission bắt buộc, mức truy cập và phạm vi công khai cho mọi nhóm endpoint đã mô tả. Endpoint Spring yêu cầu xác thực sử dụng Spring Security và `@PreAuthorize`; endpoint suy luận FastAPI là API nội bộ dịch vụ-đến-dịch vụ. Đường dẫn endpoint hiện có không thay đổi.

### Phần 8 — Yêu cầu phi chức năng

**Nội dung hiện tại được thay thế:** Quy định JWT/RBAC chung nhưng chưa có ràng buộc về khả năng bảo trì tập trung, mức độ liên quan của giao diện hoặc cách nạp permission.

**Nội dung sửa đổi:** Bắt buộc áp dụng đặc quyền tối thiểu, phòng thủ nhiều lớp, kiểm toán bất biến cho thao tác nhạy cảm, định nghĩa chính sách tập trung, không nhân bản trang/route, nạp permission từ phiên/hồ sơ hiện tại, lọc thanh bên trong bộ nhớ, không gọi API kiểm tra permission ở mỗi lần render và duy trì điều hướng gọn, phù hợp.

### Phần 9.3 — Kiểm thử chấp nhận

**Nội dung hiện tại được thay thế:** Chưa có tiêu chí chấp nhận RBAC chuyên biệt.

**Nội dung sửa đổi:** Kiểm thử bao phủ từng vai trò đã phê duyệt, nhóm và thao tác bị ẩn, từ chối URL trực tiếp, thực thi ở backend, hành vi làm mới khi nhận 401, giữ phiên khi nhận 403 và hồi quy đối với điều hướng, biểu mẫu, bộ lọc, phân trang, hộp thoại hiện có.

## 10.4. Yêu cầu mới được bổ sung

| Mã yêu cầu | Yêu cầu | Module liên quan |
|:---|:---|:---|
| FR-PORTAL-01–06 | Tái sử dụng một cổng; lọc điều hướng/thao tác; tái sử dụng route/component; backend có thẩm quyền cuối cùng. | Cổng quản trị dùng chung |
| BR-AUTHZ-02–04 | Spring Security/`@PreAuthorize`, frontend theo permission và không chỉ dựa vào bảo mật giao diện. | Phân quyền |
| FR-AUTHZ-ROUTE-01–04 | Bảo vệ URL trực tiếp, Từ chối truy cập, tái sử dụng route và tách xác thực/phân quyền. | Phân quyền |
| FR-AUTHZ-ACTION-01–03 | Kiểm soát bằng permission cho mọi thao tác thay đổi, tab, xuất dữ liệu, hoàn tiền, widget và yêu cầu API trực tiếp. | Phân quyền |
| BR-ROLE-02–04 | Danh mục `RESOURCE_ACTION`, đăng ký tập trung và một vai trò chính trong MVP. | Vai trò và Permission |
| FR-DASH-01–03 | Một dashboard dùng chung với widget được kiểm soát bằng permission; không tải widget trái phép. | Dashboard |
| NFR-PERF-03 | Nạp permission từ phiên/hồ sơ và lọc thanh bên trong bộ nhớ. | Hiệu năng |
| NFR-SEC-03–05 | Phân biệt 401/403 và tạo bản ghi kiểm toán bất biến, đầy đủ. | Bảo mật |
| NFR-MAINT-01–02 | Chính sách tập trung, không phân tán kiểm tra vai trò hoặc nhân bản trang/route. | Khả năng bảo trì |
| NFR-USE-01–02 | Chỉ hiển thị chức năng liên quan, không có nhóm rỗng, điều hướng nhất quán và gọn. | Khả năng sử dụng |
| AT-RBAC-01–12 | Bộ tiêu chí chấp nhận về hiển thị, từ chối, hành vi phiên và hồi quy theo vai trò. | Kiểm thử |

## 10.5. Nhật ký thay đổi

| Mã thay đổi | Phần SRS | Loại thay đổi | Mô tả |
|:---|:---|:---|:---|
| RBAC-001 | 2.1–2.3 | Thay thế | Thay tác nhân nội bộ dư thừa bằng bốn vai trò đã phê duyệt và cập nhật các sơ đồ. |
| RBAC-002 | 2.3.2 | Bổ sung | Bổ sung Cổng quản trị dùng chung với điều hướng theo vai trò và kiến trúc thông tin thanh bên. |
| RBAC-003 | Module 1–2 | Cập nhật | Tách xác thực/phân quyền và quy định việc thực thi ở frontend/backend cùng hành vi 401/403. |
| RBAC-004 | Module 3–4 | Cập nhật | Tập trung hóa mô hình permission và chính sách một vai trò chính trong MVP. |
| RBAC-005 | Module 5–35 | Thay thế | Ánh xạ tên vai trò cũ; `Maintenance Tech` bị ngừng sử dụng và được thay bằng `OPERATIONS_STAFF`. |
| RBAC-006 | 3.36–3.37 | Bổ sung | Bổ sung ma trận quyền truy cập menu và mô hình widget trên dashboard dùng chung. |
| RBAC-007 | 6.1 | Cập nhật | Bổ sung ràng buộc duy nhất/số lượng vai trò RBAC và metadata kiểm toán. |
| RBAC-008 | 7.1.8 | Bổ sung | Bổ sung danh mục vai trò, permission, mức truy cập và phạm vi công khai của API; không thay đổi đường dẫn. |
| RBAC-009 | 8.1, 8.4, 8.7, 8.8 | Bổ sung | Bổ sung yêu cầu về hiệu năng, bảo mật, khả năng bảo trì và khả năng sử dụng. |
| RBAC-010 | 9.3 | Bổ sung | Bổ sung kiểm thử chấp nhận và hồi quy RBAC. |

**Giả định kiểm tra:** Tên vai trò lịch sử chỉ xuất hiện trong bảng ánh xạ/nhật ký thay đổi này hoặc tại vị trí được đánh dấu rõ là đã thay thế; chúng không phải vai trò đang hoạt động trên Cổng quản trị. Không thay đổi quy tắc nghiệp vụ, thiết kế BI, tích hợp IoT, luồng thanh toán, luồng vé hoặc đường dẫn API không liên quan.
